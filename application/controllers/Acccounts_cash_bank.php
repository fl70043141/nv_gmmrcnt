...
class Acccounts_cash_bank extends CI_Controller {

	
        function __construct() {
            parent::__construct();
            $this->load->model('Acccounts_cash_bank_model'); 
        }
        ...
        function view_search($datas=''){
            $data['search_list'] = $this->Acccounts_cash_bank_model->search_result();
            $data['main_content']='acccounts_cash_bank/search_acccounts_cash_bank';  
            
            $data['account_type_list'] = array(''=>'Select No Account Type', 1=>'Cuurent',2=>'Saving Account', 3=>'Cash Book');
            $this->load->view('includes/template',$data);
	}
        
	function add(){ 
            $data['action']		= 'Add';
            $data['main_content']='acccounts_cash_bank/manage_acccounts_cash_bank'; 
...
//            echo '<pre>';            print_r($data); die;
		$add_stat = $this->Acccounts_cash_bank_model->add_db($data);
                
		if($add_stat[0]){ 
                    //update log data
                    $new_data = $this->Acccounts_cash_bank_model->get_single_row($add_stat[1]);
...
//            echo '<pre>'; print_r($data); die;
            ...
            $existing_data = $this->Acccounts_cash_bank_model->get_single_row($inputs['id']);

            $edit_stat = $this->Acccounts_cash_bank_model->edit_db($inputs['id'],$data);
            
            if($edit_stat){
                //update log data
                $new_data = $this->Acccounts_cash_bank_model->get_single_row($inputs['id']);
        ...
        function remove(){
            ...
            $existing_data = $this->Acccounts_cash_bank_model->get_single_row($inputs['id']);
            $delete_stat = $this->Acccounts_cash_bank_model->delete_db($inputs['id'],$data);
	...
	function remove2(){
            ...
            $existing_data = $this->Acccounts_cash_bank_model->get_single_row($inputs['id']);
            if($this->Acccounts_cash_bank_model->delete2_db($id)){
                //update log data
        ...
        function load_data($id){
            ...
            $data['user_data'] = $this->Acccounts_cash_bank_model->get_single_row($id); 
		...
		$data_view['search_list'] = $this->Acccounts_cash_bank_model->search_result($search_data);
                                        
		$this->load->view('acccounts_cash_bank/search_acccounts_cash_bank_result',$data_view);
	}
                                        
        function test(){
            echo '<pre>';            print_r($this->router->class); die;
//            $this->load->model('Acccounts_cash_bank_model');
//            $data = $this->Acccounts_cash_bank_model->get_single_row(1);
