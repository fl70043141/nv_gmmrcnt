...
class Items_model extends CI_Model
{
	function __construct(){	
        ...
        //    echo '<pre>';            print_r($data); die;
            $this->db->select('i.*');
            if(isset($data['item_type_id']) && $data['item_type_id']==1) $this->db->select('(select supplier_name from '.SUPPLIERS.' where id = si.supplier_id)  as supplier_name, sd.purchasing_unit, sd.secondary_unit');
            ...
            $this->db->where('i.deleted',0);
             ...
             if(isset($data['item_type_id']) && $data['item_type_id']!='') $this->db->where('i.item_type_id',$data['item_type_id']);
             if(isset($data['item_type_id']) && $data['item_type_id']==1) if(isset($data['supplier_id']) && $data['supplier_id']!='') $this->db->where('si.supplier_id',$data['supplier_id']);
...
//            echo $this->db->last_query(); die;
          ...
          public function get_single_row($id){
            $this->db->select('i.*');
            $this->db->select('ic.category_code');
...
//            echo $this->db->last_query(); die;
        ...
        public function get_item_purch_prices($item_id,$where=''){
            ...
            $this->db->select('(select supplier_name from '.SUPPLIERS.' where id = ip.supplier_id)  as supplier_name');
            $this->db->select('(select supplier_ref from '.SUPPLIERS.' where id = ip.supplier_id)  as supplier_code');
