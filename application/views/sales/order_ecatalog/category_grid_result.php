            ...
            echo '<div class="col-lg-4 col-md-4 col-sm-4 col-xs-4">
                    <div  id="'.$search['id'].'" class="thumbnail cat_click">
                        ...
                        <div class="caption">
                            ...
                            <a href="'. base_url($this->router->fetch_class().'/item_list/1/'.$search['id']).'" class="mailbox-attachment-name center">'.$search['category_name'].'</a> 
                          </div>
                        </div>
                          <a href="'. base_url($this->router->fetch_class().'/item_list/1/'.$search['id']).'"> <img src="'. base_url(CAT_IMAGES.(($search['cat_image']!='')?$search['id'].'/'.$search['cat_image']:'../default/default.jpg')).'" alt="'.$search['category_name'].'" style="width:100%;overflow: hidden"></a>
                    </div>
                  </div>';
        }
    }else{
